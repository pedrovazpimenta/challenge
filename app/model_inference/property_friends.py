...
def pf_basic_model_inference(input: classes.FPInferenceInput, hashes: tuple) -> (np.ndarray | tuple[np.ndarray, np.ndarray]):
    """
    Load a model from S3 and make predictions on a dataset.

    Args:
        input (FPInferenceInput): the input parameters for the inference.
        hashes (tuple): the hashes of the
    """
    try:
        ...
        logger.error(f"An error occurred: {e}", run_hash=hashes[0], execution_hash=hashes[1])
        return None